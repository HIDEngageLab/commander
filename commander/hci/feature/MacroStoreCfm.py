...
class MacroStoreCfm(MacroCmd):
    """
    macro data download confirmation
    """

    def __init__(self, command):
        MacroCmd.__init__(self, 'DISPLAY', GenericCmd.DIRECTION['CFM'])
    ...
    def fields(self):
        return {
            self.str_command(self.command, GenericCmd.COMMANDS, '_'): {
                'RESULT': self.str_field(self.__result, MacroCmd.RESULT),
                'MEMORY': self.memory,
            }
        }
